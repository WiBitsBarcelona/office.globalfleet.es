    ...
    <ModalBody>
      <div class="grid grid-cols-4 gap-2 mb-5 border-b-[1px] pb-2 border-slate-200">
        ...
        <div class="relative px-2 ml-auto">
          <AlertTriangleIcon class="ml-auto md:w-18 hover:mouse-pointer" :class="trip_incidences_class" /> 
          <div class="absolute inline-flex items-center justify-center w-6 h-6 text-xs font-bold text-white border-2 rounded-full -top-2 -right-3 dark:border-gray-900" :class="trip_badge_incidences_class">{{ total_new_trip_incidences }}/{{ total_trip_incidences }}</div>
        </div>
          ...
          <div class="md:flex items-center md:space-x-4 mb-3">
            <div class="flex items-center space-x-4 md:space-x-2 md:space-x-reverse">
              ...
              <div class="relative text-warning md:w-14" style="text-align: -moz-right;">
                ...
                <div v-if="element.element_type == 'Stage'" class="absolute inline-flex items-center justify-center w-6 h-6 text-xs font-bold text-white border-2 rounded-full -top-7 -right-4 dark:border-gray-900 p-3" :class="element.stage_badge_incidences_class">{{ element.total_new_stage_incidences }}/{{ element.total_stage_incidences }}</div>
              </div>
            </div>
            <!-- Action Card -->
            <div v-if="element.element_type == 'Action'" class="bg-white p-4 rounded border border-slate-200 text-slate-500 shadow ml-14 md:ml-44" :class="element.box_size">
              <div class="grid grid-cols-3 gap-2 mb-5pb-2">
                ...
                <div class="text-end">
                  <span
                    :class="element.status_class">{{ element.element_status }}</span>
                </div>
                <div class="px-2">
            ...
            <div v-if="element.element_type == 'Stage'" class="bg-white p-4 rounded border border-slate-200 text-slate-500 shadow ml-14 md:ml-44" :class="element.box_size">
              ...
              <div class="relative text-warning md:w-4" style="text-align: -moz-right;">
                <AlertTriangleIcon :class="task.task_incidences_class" />
                <div class="absolute inline-flex items-center justify-center w-6 h-6 text-xs font-bold text-white border-2 rounded-full -top-7 -right-4 dark:border-gray-900 p-3" :class="task.task_badge_incidences_class">{{ task.total_new_task_incidences }}/{{ task.total_task_incidences }}</div>
              </div>
                    </div>
                    <!-- Title -->
                    ...
                    <div class="w-full bg-white p-4 rounded border border-slate-200 text-slate-500 shadow ml-14 md:ml-44">
                      <div class="grid grid-cols-3 gap-2 mb-5pb-2">
                        <div class="col-span-2">
                          <p class="text-md font-normal leading-6 text-gray-500"><span
                              class="text-xs font-light text-gray-400">{{ $t("task") }}:</span> {{ task.task_name }}</p>
                        </div>
                        <div class="text-end">
                          <span
                            :class="task.status_class">{{ task.task_status }}</span>
                        </div>
                        <div class="px-2">
  ...
  trip.value.stages.forEach(element => {
    ...
    if (element.activity == null){
      ...
      if(element.incidents !== null){
        ...
        stage_incidences_class = 'text-gray-300';
        ...
        element.incidents.forEach( incident => {
          if(incident.stage_id == element.id)
          {
            ...
            if(incident.has_seen == 0){
              stage_incidences_class = 'text-warning';
      ...
      element.tasks.forEach( task => {
          ...
          task_incidences_class = 'text-gray-300';
          ...
          task.incidents.forEach( incident => {
            if(incident.task_id == task.id)
            {
              ...
              if(incident.has_seen == 0){
                task_incidences_class = 'text-warning';
