    ...
    <ModalBody class="px-2 py-5 text-center">
      ...
      <div class="grid grid-cols-12 gap-6 mx-3 mt-5 items-center justify-center">
        ...
        <div v-for="incidenceFile in incidenceState.incidenceFiles" :key="incidenceFile.index"
          ...
          <Dropdown class="inline-block" placement="bottom-start" :show="dropdown" @hidden="dropdown = false">
            ...
            <DropdownMenu>
              <DropdownContent>
                <DropdownItem
                  @click="tripIncidenceFile(incidenceFile.id, incidenceFile.path, incidenceFile.has_seen, incidenceFile.file_name, 0)">
                  <EyeIcon class="w-4 h-4 mr-2" /> {{ $t("incidences.Modal.file_view") }}
                </DropdownItem>
                <DropdownItem
                  @click="tripIncidenceFile(incidenceFile.id, incidenceFile.path, incidenceFile.has_seen, incidenceFile.file_name, 1)">
        ...
        <div class="col-span-12 flex mt-5 justify-center">
          <button type="button" @click="setReadedTripIncidence(trip_incidence_selected_id)"
            class="btn btn-primary w-60 mr-5" :class="{ 'hidden': trip_incidence_readed != 0 }">
